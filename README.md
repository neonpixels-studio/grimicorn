...
- **`ci.yml`** runs two jobs on pushes/PRs to `main`: a `ci` job (`lint`, `typecheck`, `test:ci`, `build`) and an `e2e` job that installs Chromium, Firefox, and WebKit and runs `test:e2e`, uploading `test-results/` on failure.
