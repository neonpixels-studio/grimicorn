...
- **`lighthouse.yml`** runs [`treosh/lighthouse-ci-action`](https://github.com/treosh/lighthouse-ci-action) (pinned to a commit SHA) against the production build, served with `.vitepress/e2e/serve-dist.mjs` — the same script the Playwright smoke test uses — so the audit sees the real generated CSP, not `vitepress preview`. That server applies only the CSP; it does not apply `netlify.toml`'s static headers (`Cache-Control`, `X-Frame-Options`, HSTS, etc.) or Netlify's response compression, so those aren't covered by this gate and the byte-size budgets below are measured against uncompressed bytes (a real Netlify response is smaller). Budgets and score thresholds (performance, accessibility, SEO, best-practices, LCP, and per-resource-type byte budgets) live in `lighthouserc.json` at the repo root; tune them there as the site's baseline shifts. The `canonical` SEO audit is skipped there since the hardcoded production canonical URL can never match the loopback host this workflow serves from. Reports are uploaded as a workflow artifact (`uploadArtifacts`); `temporaryPublicStorage` is not enabled.
