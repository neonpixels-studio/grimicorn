...
- **`lighthouse.yml`** runs [`treosh/lighthouse-ci-action`](https://github.com/treosh/lighthouse-ci-action) (pinned to a commit SHA) against the production build, served with `.vitepress/e2e/serve-dist.mjs` — the same script the Playwright smoke test uses — so the audit sees the real generated CSP, not `vitepress preview`. That server applies the CSP and gzips compressible responses (approximating Netlify's edge compression — Netlify serves Brotli to browsers that support it, so real transfers are slightly smaller than what this gate measures — so LCP/byte-budget numbers aren't inflated by artificially-uncompressed transfers); it does not apply `netlify.toml`'s other static headers (`Cache-Control`, `X-Frame-Options`, HSTS, etc.), so those aren't covered by this gate. Budgets and score thresholds (performance, accessibility, SEO, best-practices, LCP, and per-resource-type byte budgets) live in `lighthouserc.cjs` at the repo root (CommonJS, not JSON, so the budget rationale — e.g. why the LCP threshold isn't the textbook 2500ms — can live as a comment next to the number it justifies); tune them there as the site's baseline shifts. The `canonical` SEO audit is skipped there since the hardcoded production canonical URL can never match the loopback host this workflow serves from. Reports are uploaded as a workflow artifact (`uploadArtifacts`); `temporaryPublicStorage` is not enabled.
