...
- **`lighthouse.yml`** runs [`treosh/lighthouse-ci-action`](https://github.com/treosh/lighthouse-ci-action) (pinned to a commit SHA) against the production build, served with `.vitepress/e2e/serve-dist.mjs` — the same script the Playwright smoke test uses — so the audit sees the real generated CSP and headers, not `vitepress preview`. Budgets and score thresholds (performance, accessibility, SEO, best-practices, LCP, and per-resource-type byte budgets) live in `lighthouserc.json` at the repo root; tune them there as the site's baseline shifts.
