...
- **`ci.yml`** runs two jobs on pushes/PRs to `main`: a `ci` job (`lint`, `typecheck`, `test:ci`, `check:asset-version-bump`, `build`) and an `e2e` job that installs Chromium and runs `test:e2e`, uploading `test-results/` on failure. The `ci` job checks out full history (`fetch-depth: 0`) so `check:asset-version-bump` can diff the asset-version lock against the PR's base branch, not just the working tree — see the script's own comments for why that base-branch comparison (rather than a local/HEAD one) is what actually catches a hand-edited lock.
