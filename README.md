...
| `npm run check:asset-version-bump` | `node scripts/check-asset-version-bump.mjs`      | CI-only guard: on a pull request, fails if any versioned asset changed relative to the base branch without the `ASSET_CACHE_BUST` token moving forward past the base branch's locked token. No-op outside a pull request (see CI below).       |
